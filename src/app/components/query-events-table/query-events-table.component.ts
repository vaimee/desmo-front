  ...
  implements OnInit, AfterViewInit, OnDestroy
{
  ...
  private async getAllCompletedQueries(): Promise<void> {
    ...
    const queryFailedFilter = contract.filters.QueryFailed();
    const queryFailedEvents = await contract.queryFilter(queryFailedFilter);
    const mappedQueryFailedEvents = queryFailedEvents.map((event: any) => {
      const taskId = event.args.id;
      return {
        blockNumber: event.blockNumber,
        transactionHash: event.transactionHash,
        taskId: taskId,
        log: `Query with request ID ${taskId} has failed`,
      } as IQueryEvent;
    });

    // TODO: with the new contracts, this event will be generated by the Desmo contract!
    const contractHub = this.desmold.desmo['contract'];
    const queryFilterHub = contractHub.filters.RequestCreated();
    ...
    this.txList = queryCompletedEvents
      ...
      .concat(mappedQueryFailedEvents)
